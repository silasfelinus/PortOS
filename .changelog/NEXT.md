...
- **Manuscript editorial review: "Start fresh" re-run mode** — re-running the editorial pass in the manuscript editor previously always *merged*, carrying every prior comment forward. A new "Start fresh" checkbox next to "Run editorial review" clears the current open and accepted notes and shows only the new pass's findings. Your dismissed notes are always kept so a dismissed finding can't quietly resurface, and accepted fixes are already applied to the manuscript so dropping their (historical) note loses nothing. Merge stays the default.
