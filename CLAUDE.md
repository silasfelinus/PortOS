...
- **Record-creating tests must disable live peer fan-out.** Any Vitest suite that calls `createUniverse` or `createSeries` for non-ephemeral fixtures must mock `services/instances.js` with `mockNoPeers()` from `server/lib/mockPathsDataRoot.js` **and** mock `services/sharing/peerSync.js` with `mockNoPeerSync()` in the same suite. Those create paths can fire a background `autoSubscribeRecordToAllPeers` import and also trigger the `peerSync` dynamic-import path; mocking only `getPeers → []` suppresses peer discovery, but can still leave background auto-subscribe / peer-sync noise running. Only rely on `mockNoPeers()` alone when the suite provably never exercises the `peerSync` import path.
