...
- **Record-creating tests must disable live peer fan-out.** Any Vitest suite that calls `createUniverse` or `createSeries` for non-ephemeral fixtures must mock `services/instances.js` with `mockNoPeers()` from `server/lib/mockPathsDataRoot.js`. Those create paths fire a background `autoSubscribeRecordToAllPeers` import; without a `getPeers → []` guard, the post-return microtask can read the real peer registry and leak fixtures into real `data/`.
