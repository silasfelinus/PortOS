...
- [ ] [strip-stage-runhistory-from-list-payloads] **[LOW][PERF]** Strip `stages.*.runHistory` from list-shaped pipeline payloads. `sanitizeIssue` now returns the full per-stage history (up to 5 × ~600KB per text stage ≈ 12MB per maxed-out issue) on every read, including the sidebar (`listRecentIssues`) and per-series list (`listIssues`). Those views never render history, so the payload + sanitize cost is pure waste. Fix: add a `withHistory: false` flag (or a parallel `sanitizeIssueSummary`) that nulls out `runHistory` for list endpoints; detail reads (`GET /api/pipeline/issues/:id`) stay on the full sanitizer. Surfaced 2026-05-20 by /simplify during the runHistory diff-modal PR; deferred because the cap-at-5 keeps memory bounded for single-user and no list view paginates that many issues today.
