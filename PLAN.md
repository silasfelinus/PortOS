...
- [ ] [extract-upsertbyidprepend-helper-universe-builder] **Extract `upsertByIdPrepend(list, item)` helper.** The "filter out by id then prepend" upsert pattern is duplicated in three places: `client/src/pages/UniverseBuilder.jsx#handleMoveCategoryToTrunk` (~line 1393), `client/src/pages/UniverseBuilder.jsx#handleGenerateInCategory` (~line 1479), and `client/src/hooks/useUniverseAction.js` (~line 72). A two-line helper in `client/src/lib/` would dedupe all three. Surfaced by `/simplify` during the `useUniverseAction` extraction; deferred so that PR stays focused on the hook itself.
