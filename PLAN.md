...
1. **Universe Builder redesign — trunks + sub-buckets layout.** Replace the vertical-stack layout with a tabbed-trunk layout (`Bible / Cast / Places / Objects / Composites / Render`). Unify custom categories as sub-buckets under one of the 3 canon trunks via a new `kind: 'characters'|'settings'|'objects'` field on each category. Card grids with thumbnails replace the per-category accordion. Multi-phase (each phase is its own PR):
   ...
   - **Phase D follow-ups (deferred from the Phase D PR):**
     - [ ] **Extract `resolveProviderAndModel({providerId, model})` into `server/lib/promptRunner.js`.** The same 5-line `getProviderById/getActiveProvider/resolveEffectiveModel` chain now lives in `expandWorldTemplate` (`universeBuilderExpand.js`), the refine entrypoint (`universeBuilderRefine.js`), AND `promoteVariationToCanon` (`universeBuilderPromote.js`). Three identical copies — consolidate next time we touch any of them.
     - [ ] **Add `findBibleEntryByName(list, name)` to `server/lib/storyBible.js`.** The "match name OR aliases case-insensitively" pattern lives in `promoteVariationToCanon` (duplicate-collision check) and `foldRetiredCharactersBucket` (seen-set construction) in `universeBuilder.js`. Will land a third caller when the Phase C auto-sort classifier ships.
     - [x] ~~**Serialize universe write-paths server-side.**~~ **Shipped 2026-05-17** — added `queueUniverseWrite` (single file-level queue) in `server/services/universeBuilder.js`. All five mutating fns (`createUniverse`, `insertUniverseWithId`, `updateUniverse`, `deleteUniverse`, `recordRun`) now enqueue onto it. The previously-inlined queue pattern from `pipeline/issues.js`, `pipeline/series.js`, and `mediaCollections.js` was extracted to a shared `server/lib/fileWriteQueue.js` factory (`createFileWriteQueue()`) and all four services were migrated onto it — drops 36 lines of copy-paste and the next pattern-tweak is a one-place fix. The unqueued migration write inside `readState()` was also removed (race-prone vs. concurrent queued writers); the next queued mutator persists the migrated shape naturally. Per CLAUDE.md "a `Map<recordId, Promise>` is *not* enough — collapse the queue to a single tail per shared file": same JSON file ⇒ single tail, not per-id. 4 regression tests cover same-universe concurrent PATCHes, cross-universe concurrent PATCHes, queue-poison-recovery, and recordRun + updateUniverse mixed concurrency; 4 helper tests pin the factory contract.
     - [ ] **Client component tests for `CategoryEditor` promote wiring + `TrunkView` button visibility.** Assert promote button disabled when `!selectedId`, picker opens for `kind: 'other'` buckets, picker option click invokes `onPromote` with the correct `targetKind`. No client vitest infra wired for `UniverseBuilder.jsx` yet — add when the page's first spec lands.
