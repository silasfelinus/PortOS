...
1. **Universe Builder redesign — trunks + sub-buckets layout.** Replace the vertical-stack layout with a tabbed-trunk layout (`Bible / Cast / Places / Objects / Composites / Render`). Unify custom categories as sub-buckets under one of the 3 canon trunks via a new `kind: 'characters'|'settings'|'objects'` field on each category. Card grids with thumbnails replace the per-category accordion. Multi-phase (each phase is its own PR):
   ...
   - **Phase D follow-ups (deferred from the Phase D PR):**
     ...
     - [ ] **Serialize universe write-paths server-side (`updateUniverse` queue per universe id).** Phase D ships a page-level client gate (`promoting` ref) that prevents the user from triggering concurrent promotes against one universe, but the server-side write path is still vulnerable to multi-client / multi-instance concurrent writes that compute `nextCanon`/`nextVariations` from stale snapshots and clobber each other. Mirror the `issueWriteTail`-style per-record queue pattern in `server/services/pipeline/issues.js` so every `updateUniverse(id, patch)` serializes on `id`. Per CLAUDE.md "Async PATCH races on shared records — serialize writes server-side."
