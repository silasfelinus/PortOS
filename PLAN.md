...
- [x] **Duplicate evaluator spawn dedup** — shipped (2026-05-10). Root cause: `spawningTasks.delete(task.id)` ran immediately after `updateTask(in_progress)` succeeded but BEFORE the spawn call queued the agent to the runner; a concurrent `task:ready` re-emission could race past the dedup-has-check and produce a duplicate spawn. Fix wraps the spawn call in `try { spawnViaRunner|spawnDirectly } finally { spawningTasks.delete(task.id) }` (`server/services/agentLifecycle.js:536-547`). New behavioral repro + source-text regression guard in `agentLifecycle.test.js`. Full server pack green (3831 tests). Pre-existing wider issue (any throw between `spawningTasks.add` and the inner try-block also leaks the guard) noted as a separate item below.
...
- [x] **Add UI hint under FFLF mode** — shipped (2026-05-10). `renderFramePanel` in `client/src/pages/VideoGen.jsx` gained a `hint` prop rendered as a subtle accent-coloured line below the existing experimental advisory ("Tip: use keyframes that share scene geometry — same camera, same subject…"). Visually distinct from the warning so the two categories of information don't blur.
- [x] **Deprecate notapalindrome models** — shipped (2026-05-10). `ltx2_unified`, `ltx23_unified`, `ltx23_distilled_q4` carry `deprecated: true` in both `server/lib/mediaModels.js` and the `data.sample/media-models.json` seed. VideoGen and CreativeDirector dropdowns render them under a `<optgroup label="Legacy">` below the active dgrauet models. `defaultMacos` deliberately stays on `ltx23_distilled_q4` and `getDefaultVideoModelId` does NOT auto-roll forward — the dgrauet runtime requires an opt-in venv install (`INSTALL_LTX2=1 bash scripts/setup-image-video.sh`), so fresh installs need a model that works out of the box. Comment in `mediaModels.js#getDefaultVideoModelId` documents this. Eventual full removal of `runtime: 'mlx_video'` dispatch (~50 LOC in `videoGen/local.js`) deferred until dgrauet is the install-default.
...
- [x] **2a. `createBibleStore({...})` factory** — shipped (2026-05-10). Factory lives in `server/lib/storyBible.js` and derives `fileName`/`listKey`/`idSuffix` regex internally from `kind`; callers supply `idPrefix` + `dedupKey` + `primaryFields` + `editableFields` + `requireOnCreate` + optional `validateAfterUpdate` + error strings. Each writers-room domain file collapses to ~30 LOC. -281 LOC across the three files; 12 new factory tests in `storyBible.test.js`. Settings' slugline-aware dedup and "blank-both → reject" guard go through the `dedupKey` callback and `validateAfterUpdate` respectively.
- [x] **2b. Share zod bible-entry schemas between writers-room + pipeline routes** — shipped (2026-05-10). `server/lib/validation.js` re-exports `characterBibleCreateSchema` / `settingBibleCreateSchema` / `objectBibleCreateSchema` under kind-neutral names (the writers-room name `writersRoomCharacterCreateSchema` etc. stay for the existing route's import). Pipeline `routes/pipeline.js` extends those with its own back-compat fields (`description` legacy alias, `imageRefs`, optional `id`) plus `.passthrough()` so sanitizer-emitted fields (evidence/firstAppearance/source/timestamps) round-trip without strict-mode rejection. The loose `bibleEntrySchema = z.record(z.string(), z.any())` is gone. `settingBibleCreateSchema` is the un-refined `ZodObject` (the refined `writersRoomSettingCreateSchema` is the route-layer enforcer for the WR endpoint only); the "name OR slugline" rule is enforced at the sanitizer/store layer (`storyBible.js#sanitizeSetting` drops blank-both entries) — single source of truth.
...
- [x] **4c. `client/src/hooks/useAsyncAction.js`** — shipped (2026-05-10). Hook returns `[run, running]`. Migrated `ProseStage.jsx#handleExtract`, `TextStagePanel.jsx#handleSave` + `#handleGenerate` (the latter OR'd with externally-synced `serverGenerating` for auto-runs), and `EpisodeVideoStage.jsx#submit`. `StoryboardsStage.jsx` deliberately stays on its existing pattern — `savingIdx` and `extractingFrom` are keyed loading states, not booleans; a keyed variant would be a bigger abstraction. Post-unmount setState guard with `mountedRef` deferred to a future general DRY pass (YAGNI for the current stable parent components).
