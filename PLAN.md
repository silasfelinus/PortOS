...
- [ ] [clear-pending-sheet-slot-on-entry-delete] **Prune `_latestPendingByCharacter` on character / universe delete.** Map at `server/services/universeCharacterSheet.js` (search `_latestPendingByCharacter`) grows bounded by characters ever rendered, but stale slot entries persist across the process lifetime. Wire a `clearPendingSheetSlot(universeId, entryId)` helper into the character-delete + universe-delete paths once those exist. Deferred from `[character-bible-simplify-followups]` because the delete paths don't exist yet.
...
- [ ] [extract-migration-scaffolding-into-scripts] **Extract migration scaffolding into `scripts/migrations/_lib.js`.** Migrations 003, 006, 019, and now 023 all implement the same hash-driven prompt-replace pattern (~75 lines of boilerplate each). Lift to a shared helper; next migration becomes ~15 lines. Migration 023 was the fourth migration to clone the scaffold — the next migration after 023 should block on this extraction first; the boilerplate cost is now larger than typical content changes.
- [ ] [extract-migration-test-helpers] **Extract migration test scaffolding into `scripts/migrations/_testHelpers.js`.** Companion to `[extract-migration-scaffolding-into-scripts]`. `019.test.js` and `023.test.js` share near-verbatim setup: a newline-normalizing `md5`, a `repoRoot` + `sampleBody(filename)` helper, a `customizedBody(filename)` fixture, a `mkdtempSync`-based `beforeEach` that seeds `data.sample/`, and five of the test cases themselves (no-op-missing / idempotent / drift-catch / synthetic-old-upgrade / customized-skip / shape-consistency). Lift `{ md5, repoRoot, sampleBody, customizedBody, makeSandbox(prefix) }` and a shared describe-block builder so each per-migration test only carries migration-specific assertions. Land alongside `_lib.js` as one cleanup PR. Surfaced by /simplify during `[resolve-issues-inherits-verify-gaps-verify-the]`.
