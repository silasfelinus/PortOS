...
- [ ] [drop-redundant-getissue-precheck-comic-cover-render] **[LOW][EFF]** Drop the `prepare: (req) => issuesSvc.getIssue(req.params.id)` precheck from the two comic-issue cover-render routes (`server/routes/pipeline.js:1518` + `:1532`). The follow-up `enqueueComicCover` / `enqueueComicBackCover` already calls `loadBibleContext(issueId)` (`server/services/pipeline/visualStages.js:401`), whose first step at `loadBibleContext`:141 is the same `getIssue(issueId)` — same `ERR_NOT_FOUND` → 404 mapping fires from there. The precheck is a duplicate `data/issues.json` read on every render request, and the inline comment's "before we spend the bible-context load" rationale is misleading because `loadBibleContext`'s first step IS that `getIssue`. Removing it also lets us drop the `prepare` knob from `makeCoverRenderHandler` if no other future caller needs it. Surfaced by /simplify during `[extract-cover-render-handler-factory]`; deferred to keep that PR a pure behavior-preserving refactor.
