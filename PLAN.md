...
1. **Universe Builder redesign — trunks + sub-buckets layout.** Replace the vertical-stack layout with a tabbed-trunk layout (`Bible / Cast / Places / Objects / Composites / Render`). Unify custom categories as sub-buckets under one of the 3 canon trunks via a new `kind: 'characters'|'settings'|'objects'` field on each category. Card grids with thumbnails replace the per-category accordion. Multi-phase (each phase is its own PR):
   ...
   - **Phase D follow-ups (deferred from the Phase D PR):**
     - [ ] [client-component-tests-for-categoryeditor-promote] **Client component tests for `CategoryEditor` promote wiring + `TrunkView` button visibility.** Assert promote button disabled when `!selectedId`, picker opens for `kind: 'other'` buckets, picker option click invokes `onPromote` with the correct `targetKind`. No client vitest infra wired for `UniverseBuilder.jsx` yet — add when the page's first spec lands.
   - **Phase C follow-ups (deferred from the Phase C PR):**
     - [ ] [unify-canoncard-variation-card-into-a-single] **Unify `CanonCard` + variation card into a single `EntryCard`.** Phase C reuses `CanonCard` for canon and `CategoryEditor` for variations inside the same TrunkView. Visual consistency is achieved through shared Tailwind classes; an actual single-component unification (with thumbnail-from-`primaryImageRef`-or-recent-render) is the Phase D card-extraction task.
     ...
     - [ ] [extract-builduniversestylecontext-helper] **Extract `buildUniverseStyleContext(universe, options)` helper.** The `LOGLINE / STYLE NOTES / EMBRACE INFLUENCES` prompt prefix is duplicated verbatim in three places: `server/services/universeBuilderPromote.js#buildPromotePrompt`, `server/services/universeBuilderAutoSort.js#buildAutoSortPrompt`, and a near-twin in `server/services/universeBuilderExpand.js` (adds `premise`). A single helper in `server/services/universeBuilder.js` (next to `joinInfluenceList`) would consolidate. Pipeline callsites at `server/services/pipeline/visualStages.js` and `pipeline/seriesTitleLogo.js` hand-roll variants of the same shape — fold them in too.
