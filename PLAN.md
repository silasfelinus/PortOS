...
- [ ] [audit-remaining-getissue-prechecks-in-pipeline-routes] **[LOW][EFF]** Audit the remaining `issuesSvc.getIssue(req.params.id).catch((err) => { throw mapServiceError(err); })` precheck pattern at `server/routes/pipeline.js:601, 683, 759, 785, 799, 1239, 1277, 1348, 1580, 1741`. Several of these handlers call services that themselves do `getIssue` internally on the first read (e.g. `generateStage` ~line 1267; `enqueueVisualImage` ~line 1729) — same pattern as the comic-cover routes we just consolidated, where the precheck was a duplicate `data/issues.json` read with no error-mapping difference. For each site, trace the called service to verify whether its first persisted read is `getIssue(id)` (or an equivalent `PIPELINE_ISSUE_NOT_FOUND` raiser) and drop the precheck where it is strictly redundant; leave precheck-only-error-shape cases alone. Surfaced by /simplify reuse-agent during `[drop-redundant-getissue-precheck-comic-cover-render]`; deferred to keep that PR a one-route consolidation.

- [ ] [cover-render-factory-target-binding] **[LOW][CODE]** Collapse `makeCoverRenderHandler`'s 7-field config into a "target binding" object so volume / comic dispatch is one symbol per route. Today `applyWrite` + `buildResponse` are independently passed and a comic config could nonsensically mix `applyWrite: updateVolumeSeason` with `buildResponse: buildVolumeCoverResponse`. A `volumeTarget` / `comicTarget` binding (`{ update, buildResponse }`) makes invalid combos unrepresentable and shrinks the four route configs to `{ schema, slotField, scriptField, target }`. Also: promote `'cover' | 'backCover'` to a `COVER_TARGETS` enum in `server/services/pipeline/owners.js` and derive `scriptField` from it so the parallel `scriptField` field can drop entirely. Surfaced by /simplify during `[extract-cover-render-handler-factory]`; deferred because there are only two bindings (volume / comic) and no third caller is in sight, so today's parameter shape is fine — pick this up when a fifth cover-render variant appears.

### v2.1.0 pre-release review residue (deferred from main→release multi-agent review, 2026-05-16)

- [ ] [agent-tui-spawn-truncation-warn-parity] **[LOW][AI-DISPATCH]** `server/services/agentTuiSpawning.js:177-179` (and the `rawBuffer` slice at line ~273-274) silently slice the head of the same buffers when they cross `OUTPUT_BUFFER_HEADROOM` / `RAW_BUFFER_HEADROOM`, with no warning and no agent-metadata flag. The companion one-shot runner now sets `outputBufferTruncated` and surfaces it through the run record (see `[med-ai-dispatch-server-lib-tuipromptrunner-js-198]`). Long-running CoS agents using `agentTuiSpawning.js` mostly stream to `output.txt` via `appendAgentOutputLines`, so the buffer overflow here is less load-bearing — but the same warn-once + agent-metadata flag would close the visibility gap. Deferred from `[med-ai-dispatch-server-lib-tuipromptrunner-js-198]` to keep that PR scoped.
...
- [ ] [legacy-series-canon-orphan-universe-leak-on-throw] **[LOW][SHARING]** `server/services/pipeline/migrateSeriesCanon.js#applyLegacySeriesCanonToUniverse` — when the helper hits a transient I/O failure between `createUniverse(...)` and `updateUniverse(universeId, patch)` (or any subsequent write), the orphan universe is persisted on disk but the helper throws. On retry the manifest reprocesses, sees `s.universeId` still unset (bucket record isn't mutated), and mints a SECOND auto-migrated universe. Canon data isn't lost (the manifest stays retryable via `legacyCanonPendingFailures`), but empty `"<series-name> (auto-migrated)"` universes accumulate per retry pass. Fix needs a deterministic universe id derived from the importing series (e.g. `uni-import-<seriesHash>` or persisting the chosen id back to the bucket sidecar) so the second creation hits `ERR_DUPLICATE` and the helper reuses the existing universe. Deferred from `[med-sharing-server-services-sharing-version-js]` because the fix touches the universe identity model — out of scope for the importer-side migration salvage PR. Surfaced 2026-05-20 in codex review iter 4.

- [ ] [universebuilder-collection-hook-flaky-batch-enrich] **[MED][TESTS]** `server/services/universeBuilderCollectionHook.test.js > "enriches every sidecar in a batch — all N images get tagged"` still fails intermittently when the full server suite runs in parallel, even after the 2026-05-19 fix that migrated `enrichSidecar` to `atomicWrite` (`[imagegen-local-defer-module-load-side-effects]` PR). Passes 100% in isolation (`vitest run services/universeBuilderCollectionHook.test.js` — 8/8). Suggests a cross-file race that survives the temp+rename atomicity — likely shared `PATHS.data` Proxy / mocked `mediaCollections` state colliding with a sibling test that drains the same hook queue concurrently. Reproduce by running the full server `vitest run` 10× and grepping for FAIL; instrument the batch enrich path to log timing and identify the contending writer. Surfaced 2026-05-20 during the legacy-series-canon import PR.
