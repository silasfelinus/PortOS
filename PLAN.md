...
- [ ] [sharing-v2-per-sender-subscription-filenames] **Per-sender subscription filenames.** Today `subscriptionFilename()` returns `sub-<kind>-<recordId>.json` (`server/services/sharing/manifest.js:194`); extend to `sub-<kind>-<recordId>-<senderInstanceId>.json` so two peers sharing the same record don't collide. Tombstone-based item removals already shipped for annotations (`server/services/sharing/annotationsSync.js:56`).
