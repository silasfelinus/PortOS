...
function Monument({ monument, shimmerRef, isShimmer, dayMix = 0 }) {
  ...
  return (
    <group position={position}>
      ...
      <CityLabel position={[0, height + 1.4, 0]} fontSize={isSpire ? 0.85 : 0.7} color={color} dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9} fillOpacity={opacity}>
        {monument.title}
      </CityLabel>
      <CityLabel position={[0, height + 0.7, 0]} fontSize={0.55} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9} fillOpacity={opacity}>
      ...
      {forest.clusters.map((cluster) => (
        <group key={cluster.spire.id}>
          ...
          {cluster.children.map((child) => (
            <group key={child.id}>
              ...
              {child.descendantCount > 0 && (
                <CityLabel position={[child.position[0], child.height + 1.9, child.position[2]]} fontSize={0.42} color="#64748b" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
          ...
          {cluster.childOverflow > 0 && (
            <CityLabel position={[cluster.spire.position[0], cluster.spire.height + 2.1, cluster.spire.position[2]]} fontSize={0.5} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={9}>
...
export default function CityGoalMonuments({ goals, settings }) {
  ...
  return (
    <group>
      ...
      <CityLabel position={[base[0], 16, base[2] - 3]} fontSize={1.4} color="#22c55e" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
        GOALS
      </CityLabel>
      <CityLabel position={[base[0], 15, base[2] - 3]} fontSize={0.8} color="#94a3b8" dayMix={dayMix} anchorX="center" anchorY="middle" font={PIXEL_FONT_URL} maxWidth={30}>
