...
// Stable non-negative hash of a string (same algorithm as Borough.jsx), used to
// give each connection deterministic per-link variation without Math.random so
// the topology is reproducible and testable.
export function hashFlowKey(str) {
  let hash = 0;
...
export function computeFlowConnections({ positions, activeIds, agentIds, maxNeighbors = FLOW.maxNeighbors } = {}) {
  ...
  for (let i = 0; i < entries.length; i++) {
    ...
    for (let n = 0; n < take; n++) {
      ...
      const variation = (hashFlowKey(key) % 100) / 100; // 0..1, deterministic per link
      conns.push({
        key,
        start: [a.x, 0.5, a.z],
        end: [b.x, 0.5, b.z],
