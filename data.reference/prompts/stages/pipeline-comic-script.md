...
Terse roster of the linked Universe Builder's named canon — use as continuity anchors for the panels you draft. Do not contradict; you may name-check but don't invent new attributes for them.
...
- **Every page MUST contain 4–6 discrete `Panel N` blocks**, each carrying its own `Description:`, `Caption:`, `Dialogue:`, and `SFX:` fields (use `(none)` for an empty caption/SFX rather than dropping the field). The only exception is a deliberate splash — still written as a single `Panel 1` block with the four fields, not as loose page-level prose.
