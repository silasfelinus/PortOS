// /ecosystem.unraid.config.cjs
const baseConfig = require('./ecosystem.config.cjs')

const baseApps = Array.isArray(baseConfig) ? baseConfig : baseConfig.apps || []
...
const apps = baseApps
  ...
  .map((app) => ({
    ...
    env: {
      ...(app.env || {}),
      NODE_ENV: 'production',
      HOST: app.name === 'portos-server' ? '0.0.0.0' : app.env?.HOST || '127.0.0.1',
...
if (!apps.length) {
  throw new Error(
    `No PM2 apps selected. PORTOS_UNRAID_APPS=${process.env.PORTOS_UNRAID_APPS || defaultApps.join(',')}`,
  )
}
