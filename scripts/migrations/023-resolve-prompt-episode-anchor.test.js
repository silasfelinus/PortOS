...
// Lookup the real data.sample file. Use fileURLToPath so a checkout under a
// path with spaces decodes correctly.
...
describe('migration 023 — resolve prompt episode-synopsis anchor', () => {
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-023-'));
  ...
  it('NEW_SHIPPED_MD5 matches the live data.sample body (drift catch)', () => {
    ...
    for (const filename of Object.keys(NEW_SHIPPED_MD5)) {
      expect(md5(sampleBody(filename))).toBe(NEW_SHIPPED_MD5[filename]);
    }
  });
