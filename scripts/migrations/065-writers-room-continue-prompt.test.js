...
import migration from './065-writers-room-continue-prompt.js';

const STAGE = 'writers-room-continue';
const readJson = (path) => JSON.parse(readFileSync(path, 'utf-8'));

describe('migration 065 — seed Writers Room live-continuation prompt', () => {
  let rootDir;
  let installedStagesDir;
  let installedConfigPath;

  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-065-'));
    ...
    const refConfig = { stages: { [STAGE]: { name: STAGE, model: 'quick', returnsJson: true, variables: [] } } };
  ...
  it('is idempotent and preserves a user-customized installed prompt', async () => {
    ...
    writeFileSync(customPath, '# CUSTOM continuation\n');
    await migration.up({ rootDir });
