...
const FILENAME = 'manuscript-reformat.md';
const STAGE_KEY = 'manuscript-reformat';
...
export default {
  async up({ rootDir }) {
    ...
    if (exists) {
      console.log(`📝 manuscript-reformat prompt: already present`);
    } else {
      const sampleExists = await access(samplePath, constants.F_OK).then(() => true, () => false);
      if (!sampleExists) {
        console.warn(`⚠️  manuscript-reformat: sample missing for ${FILENAME} — skipping copy`);
      } else {
        try {
          await copyFile(samplePath, dataPath);
          console.log(`✅ seeded ${FILENAME}`);
        } catch (err) {
          console.warn(`⚠️  manuscript-reformat: copy failed for ${FILENAME}: ${err.message}`);
        }
      }
    }
    ...
    if (!sampleConfigExists) {
      console.warn('⚠️  manuscript-reformat: data.reference stage-config.json missing — cannot resolve entry; skipping config write');
      return;
    }
    try {
      ...
      if (installed.stages[STAGE_KEY]) {
        console.log(`📝 manuscript-reformat stage-config: already present`);
        return;
      }
      if (!sample?.stages?.[STAGE_KEY]) {
        console.warn(`⚠️  manuscript-reformat: sample stage-config missing ${STAGE_KEY} — skipping`);
        return;
      }
      ...
      console.log(`📝 manuscript-reformat stage-config (${action}): 1 added`);
    } catch (err) {
      console.warn(`⚠️  manuscript-reformat: stage-config merge failed: ${err.message}`);
    }
  },
};
