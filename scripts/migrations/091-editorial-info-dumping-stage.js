...
const FILENAME = 'pipeline-editorial-info-dumping.md';
const STAGE_KEY = 'pipeline-editorial-info-dumping';
...
export default {
  async up({ rootDir }) {
    ...
    if (exists) {
      console.log(`📝 pipeline-editorial-info-dumping prompt: already present`);
    } else {
      const sampleExists = await access(samplePath, constants.F_OK).then(() => true, () => false);
      if (!sampleExists) {
        console.warn(`⚠️  pipeline-editorial-info-dumping: sample missing for ${FILENAME} — skipping copy`);
      } else {
        try {
          await copyFile(samplePath, dataPath);
          console.log(`✅ seeded ${FILENAME}`);
        } catch (err) {
          console.warn(`⚠️  pipeline-editorial-info-dumping: copy failed for ${FILENAME}: ${err.message}`);
        }
      }
    }
    ...
    if (!sampleConfigExists) {
      console.warn('⚠️  pipeline-editorial-info-dumping: data.reference stage-config.json missing — cannot resolve entry; skipping config write');
      return;
    }
    try {
      ...
      if (installed.stages[STAGE_KEY]) {
        console.log(`📝 pipeline-editorial-info-dumping stage-config: already present`);
        return;
      }
      if (!sample?.stages?.[STAGE_KEY]) {
        console.warn(`⚠️  pipeline-editorial-info-dumping: sample stage-config missing ${STAGE_KEY} — skipping`);
        return;
      }
      ...
      console.log(`📝 pipeline-editorial-info-dumping stage-config (${action}): 1 added`);
    } catch (err) {
      console.warn(`⚠️  pipeline-editorial-info-dumping: stage-config merge failed: ${err.message}`);
    }
  },
};
