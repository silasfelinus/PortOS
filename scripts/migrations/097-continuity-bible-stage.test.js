...
import migration from './097-continuity-bible-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 093/095) — no MD5 bookkeeping, so the
...
const BODY = '# Continuity bible\n\nshipped body\n';

describe('migration 097 — seed continuity-bible stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  const seedReference = ({ withConfig = true } = {}) => {
    ...
    if (withConfig) {
      ...
      writeFileSync(
        refConfigPath,
        JSON.stringify({ stages: { [STAGE_KEY]: { name: STAGE_KEY, description: 'desc', model: 'heavy', returnsJson: true, variables: [] } } }, null, 2) + '\n',
      );
    }
  };

  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-097-'));
