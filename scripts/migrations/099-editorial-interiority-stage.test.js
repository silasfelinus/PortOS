...
import migration from './099-editorial-interiority-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 096/097) — no MD5 bookkeeping, so the
...
const BODY = '# Protagonist interiority\n\nshipped body\n';

describe('migration 099 — seed editorial-interiority stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-099-'));
