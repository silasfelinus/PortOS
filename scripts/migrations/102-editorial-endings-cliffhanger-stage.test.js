...
import migration from './102-editorial-endings-cliffhanger-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 100) — no MD5 bookkeeping, so the
...
const BODY = '# Chapter-ending cliffhangers\n\nshipped body\n';

describe('migration 102 — seed editorial-endings-cliffhanger stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-102-'));
