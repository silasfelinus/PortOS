...
import migration from './105-editorial-dialogue-pleasantries-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 101) — no MD5 bookkeeping, so the
// seed / no-clobber behavior is asserted directly.

const FILENAME = 'pipeline-editorial-dialogue-pleasantries.md';
const STAGE_KEY = 'pipeline-editorial-dialogue-pleasantries';
const BODY = '# Empty greeting\n\nshipped body\n';

describe('migration 105 — seed editorial-dialogue-pleasantries stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-105-'));
