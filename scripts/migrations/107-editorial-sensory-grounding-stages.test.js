...
// First-shipment seed migration (mirrors 094/095's two-file seed test) — no MD5
// bookkeeping, so the seed/no-clobber behavior is asserted directly.

const FILES = [
  { filename: 'pipeline-editorial-sensory-balance.md', stageKey: 'pipeline-editorial-sensory-balance', body: '# Sensory balance\n\nshipped body\n', returnsJson: true },
...
describe('migration 107 — seed scene-grounding editorial stages', () => {
  ...
  it('seeds both .md templates and merges both stage-config entries on a fresh install', async () => {
    ...
    expect(installed.stages['pipeline-editorial-sensory-balance']).toMatchObject({ returnsJson: true });
    expect(installed.stages['pipeline-editorial-white-room']).toMatchObject({ returnsJson: true });
  });
