...
import migration from './109-editorial-arc-transitions-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 099) — no MD5 bookkeeping, so the
...
const BODY = '# Character-arc transitions\n\nshipped body\n';

describe('migration 109 — seed editorial-arc-transitions stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-109-'));
