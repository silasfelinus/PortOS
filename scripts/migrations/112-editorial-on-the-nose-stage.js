...
const FILENAME = 'pipeline-editorial-on-the-nose.md';
const STAGE_KEY = 'pipeline-editorial-on-the-nose';
...
export default {
  async up({ rootDir }) {
    ...
    if (exists) {
      console.log(`📝 pipeline-editorial-on-the-nose prompt: already present`);
    } else {
      const sampleExists = await access(samplePath, constants.F_OK).then(() => true, () => false);
      if (!sampleExists) {
        console.warn(`⚠️  pipeline-editorial-on-the-nose: sample missing for ${FILENAME} — skipping copy`);
      } else {
        try {
          await copyFile(samplePath, dataPath);
          console.log(`✅ seeded ${FILENAME}`);
        } catch (err) {
          console.warn(`⚠️  pipeline-editorial-on-the-nose: copy failed for ${FILENAME}: ${err.message}`);
        }
      }
    }
    ...
    if (!sampleConfigExists) {
      console.warn('⚠️  pipeline-editorial-on-the-nose: data.reference stage-config.json missing — cannot resolve entry; skipping config write');
      return;
    }
    try {
      ...
      if (installed.stages[STAGE_KEY]) {
        console.log(`📝 pipeline-editorial-on-the-nose stage-config: already present`);
        return;
      }
      if (!sample?.stages?.[STAGE_KEY]) {
        console.warn(`⚠️  pipeline-editorial-on-the-nose: sample stage-config missing ${STAGE_KEY} — skipping`);
        return;
      }
      ...
      console.log(`📝 pipeline-editorial-on-the-nose stage-config (${action}): 1 added`);
    } catch (err) {
      console.warn(`⚠️  pipeline-editorial-on-the-nose: stage-config merge failed: ${err.message}`);
    }
  },
};
