...
import migration from './116-editorial-unmodeled-names-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 115) — no MD5 bookkeeping, so the
...
const BODY = '# Unmodeled proper nouns used as character names\n\nshipped body\n';

describe('migration 116 — seed editorial-unmodeled-names stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-116-'));
