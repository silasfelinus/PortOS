...
import migration from './118-editorial-appearance-continuity-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 117) — no MD5 bookkeeping, so the
...
const BODY = '# Pipeline — Editorial Check: Storyboard appearance / prop continuity\n\nshipped body\n';

describe('migration 118 — seed editorial-appearance-continuity stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-118-'));
