...
import migration from './129-editorial-timeline-contradiction-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 109) — no MD5 bookkeeping, so the
...
const BODY = '# Timeline / canon contradiction\n\nshipped body\n';

describe('migration 129 — seed editorial-timeline-contradiction stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-129-'));
