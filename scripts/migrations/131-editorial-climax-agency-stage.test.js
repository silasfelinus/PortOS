...
import migration from './131-editorial-climax-agency-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 130) — no MD5 bookkeeping, so the
...
const BODY = '# Climax / resolution power\n\nshipped body\n';

describe('migration 131 — seed editorial-climax-agency stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-131-'));
