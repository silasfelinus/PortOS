...
import migration from './132-editorial-reaction-proportionality-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 131) — no MD5 bookkeeping, so the
// seed / no-clobber behavior is asserted directly.

const FILENAME = 'pipeline-editorial-reaction-proportionality.md';
const STAGE_KEY = 'pipeline-editorial-reaction-proportionality';
const BODY = '# Emotional beat proportionality\n\nshipped body\n';

describe('migration 132 — seed editorial-reaction-proportionality stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-132-'));
