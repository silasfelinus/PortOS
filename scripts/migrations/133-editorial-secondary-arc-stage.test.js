...
import migration from './133-editorial-secondary-arc-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 132) — no MD5 bookkeeping, so the
// seed / no-clobber behavior is asserted directly.

const FILENAME = 'pipeline-editorial-secondary-arc.md';
const STAGE_KEY = 'pipeline-editorial-secondary-arc';
const BODY = '# Secondary-character arcs\n\nshipped body\n';

describe('migration 133 — seed editorial-secondary-arc stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-133-'));
