...
import migration from './134-editorial-voice-consistency-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 133) — no MD5 bookkeeping, so the
...
const BODY = '# Narrative voice / tone consistency\n\nshipped body\n';

describe('migration 134 — seed editorial-voice-consistency stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-134-'));
