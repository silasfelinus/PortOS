/**
 * Seed the `pipeline-editorial-arc-regression` stage into existing installs
 * (#1619).
 *
 * Mirrors `140-editorial-pacing-escalation-stage.js`: copies the `.md` template
 ...
 * running `update.sh`) would otherwise leave the new arc-regression stage
 * unseeded and the `arc.regression` editorial check would throw "Stage not
...
const FILENAME = 'pipeline-editorial-arc-regression.md';
const STAGE_KEY = 'pipeline-editorial-arc-regression';
...
export default {
  async up({ rootDir }) {
    ...
    if (exists) {
      console.log(`📝 pipeline-editorial-arc-regression prompt: already present`);
    } else {
      const sampleExists = await access(samplePath, constants.F_OK).then(() => true, () => false);
      if (!sampleExists) {
        console.warn(`⚠️  pipeline-editorial-arc-regression: sample missing for ${FILENAME} — skipping copy`);
      } else {
        try {
          await copyFile(samplePath, dataPath);
          console.log(`✅ seeded ${FILENAME}`);
        } catch (err) {
          console.warn(`⚠️  pipeline-editorial-arc-regression: copy failed for ${FILENAME}: ${err.message}`);
        }
      }
    }
    ...
    if (!sampleConfigExists) {
      console.warn('⚠️  pipeline-editorial-arc-regression: data.reference stage-config.json missing — cannot resolve entry; skipping config write');
      return;
    }
    try {
      ...
      if (installed.stages[STAGE_KEY]) {
        console.log(`📝 pipeline-editorial-arc-regression stage-config: already present`);
        return;
      }
      if (!sample?.stages?.[STAGE_KEY]) {
        console.warn(`⚠️  pipeline-editorial-arc-regression: sample stage-config missing ${STAGE_KEY} — skipping`);
        return;
      }
      ...
      console.log(`📝 pipeline-editorial-arc-regression stage-config (${action}): 1 added`);
    } catch (err) {
      console.warn(`⚠️  pipeline-editorial-arc-regression: stage-config merge failed: ${err.message}`);
    }
  },
};
