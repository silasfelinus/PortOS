...
import migration from './141-editorial-arc-regression-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 140) — no MD5 bookkeeping, so the
...
const BODY = '# Character-arc regression / premature closure\n\nshipped body\n';

describe('migration 141 — seed editorial-arc-regression stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-141-'));
