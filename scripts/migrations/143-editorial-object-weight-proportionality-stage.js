/**
 * Seed the object weight-proportionality editorial-check stage into existing
 * installs (#1624).
 *
 * Mirrors `142-editorial-interiority-balance-stage.js`: copies the `.md`
 ...
 * stage unseeded and the `objects.weight-proportionality` editorial check would
...
const STAGES = [
  'pipeline-editorial-object-weight-proportionality',
];
...
export default {
  async up({ rootDir }) {
    ...
    if (!sampleConfigExists) {
      console.warn('⚠️  143-editorial-object-weight-proportionality-stage: data.reference stage-config.json missing — cannot resolve entries; skipping config write');
      return;
    }
    try {
      ...
      for (const stageKey of STAGES) {
        ...
        if (!sample?.stages?.[stageKey]) {
          console.warn(`⚠️  143-editorial-object-weight-proportionality-stage: sample stage-config missing ${stageKey} — skipping`);
          continue;
        }
      ...
      if (added === 0) {
        console.log('📝 143-editorial-object-weight-proportionality-stage: no stage-config entries to add');
        return;
      }
      ...
      console.log(`📝 143-editorial-object-weight-proportionality-stage stage-config (${action}): ${added} added`);
    } catch (err) {
      console.warn(`⚠️  143-editorial-object-weight-proportionality-stage: stage-config merge failed: ${err.message}`);
    }
  },
};
