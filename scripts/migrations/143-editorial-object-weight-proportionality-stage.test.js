...
import migration from './143-editorial-object-weight-proportionality-stage.js';

// scripts/migrations/<this> → ../.. is the repo root (matches _testHelpers.js).
const repoRoot = join(dirname(fileURLToPath(import.meta.url)), '..', '..');

// First-shipment seed migration (mirrors 142's seed test) — no MD5 bookkeeping,
// so the seed/no-clobber behavior is asserted directly.

const FILES = [
  { filename: 'pipeline-editorial-object-weight-proportionality.md', stageKey: 'pipeline-editorial-object-weight-proportionality', body: '# Object weight proportionality\n\nshipped body\n', returnsJson: true },
];

describe('migration 143 — seed object weight-proportionality editorial stage', () => {
  let rootDir;
  let stagesDir;
  let refStagesDir;
  ...
  beforeEach(() => {
    rootDir = mkdtempSync(join(tmpdir(), 'migration-143-'));
  ...
  it('seeds the .md template and merges the stage-config entry on a fresh install', async () => {
    ...
    expect(installed.stages['pipeline-editorial-object-weight-proportionality']).toMatchObject({ returnsJson: true });
  });
