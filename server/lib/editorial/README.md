...
| `shotContinuity.js` | Pure, dependency-free storyboard shot-continuity primitives for the deterministic `visual.shot-continuity` check (#1315): `findAxisReversals` (180°-rule axis flips across a continuity-linked shot pair using `screenDirection`) and `findShotTypeMonotony` (a scene whose classified shots all share one `shotType`). Also `summarizeStoryboardShots` (#1466) — renders the collected `{ issueNumber, scene }` storyboard list into a compact per-scene shot block for the LLM `visual.eyeline-match` sibling (gaze reciprocity / eyeline-vs-screen-direction). Reads the shot-grammar fields from `server/lib/shotGrammar.js`. The remaining judgment case (appearance/prop continuity) is an LLM-sibling follow-up. |
