...
router.post('/merge/preview', asyncHandler(async (req, res) => {
  ...
  const preview = await mergeUniverses(body.survivorId, body.loserId, body.fieldChoices, { dryRun: true })
...
router.post('/merge', asyncHandler(async (req, res) => {
  const body = validateRequest(mergeSchema, req.body ?? {});
  const result = await mergeUniverses(body.survivorId, body.loserId, body.fieldChoices)
    .catch((err) => { throw mapServiceError(err); });
  res.json(result);
}));
