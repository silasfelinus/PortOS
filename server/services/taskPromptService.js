 ...
 * Task Prompt Service
 *
 * Owns the default prompt catalog and prompt-resolution logic for scheduled
 * improvement tasks. Split out of taskSchedule.js (issue #744) so the prompt
 * data and getters live in one place, separate from the schedule/interval
 ...
 * Do NOT change prompt defaults here without bumping PROMPT_VERSIONS and preserving
 * the prior default in PREVIOUS_DEFAULT_PROMPTS.
 *
 * taskSchedule.js re-exports the public prompt API (getDefaultPrompt, getTaskPrompt,
 * getStagePrompt, PROMPT_VERSIONS, REFERENCE_WATCH_AUDITED_VERSION) so existing
 * importers are unaffected by this split.
 */
...
// getTaskInterval lives in taskSchedule.js, which statically imports the prompt
// constants from this module. Lazy-import it here (call-time, not load-time) so the
// two modules don't form a static circular import — mirroring how cos.js and
// referenceRepos.js already dynamically import taskSchedule.js.
async function getTaskInterval(taskType) {
  const { getTaskInterval: load } = await import('./taskSchedule.js');
